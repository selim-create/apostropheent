...
function legacyVideo(url: string): ApiWorkVideo | null {
  ...
  return {
    ...
    url,
    attachment_id: 0,
    source_type: /youtube|youtu\.be/i.test(url) ? 'youtube' : /vimeo/i.test(url) ? 'vimeo' : 'file',
...
export default function WorkVideos({ videos, legacyUrl, workTitle, label }: {
  videos?: ApiWorkVideo[];
  legacyUrl?: string;
  workTitle: string;
  label: string;
  ...
  return (
    <section className={styles.section} aria-label={`${workTitle} ${label.toLowerCase()}`} data-reveal>
      ...
      <div className={`${styles.grid} ${layoutClass}`}>
        {items.map((video, index) => {
          ...
          return (
            <article className={`${styles.item} ${orientationClass}${video.featured ? ` ${styles.featured}` : ''}`} key={`${video.id}-${video.url}`}>
              <div className={styles.media}>
                {embed ? (
                  <iframe src={embed} title={title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen />
                ) : (
                  <WorkVideo src={video.url} title={title} poster={video.poster?.url || undefined} />
                )}
              </div>
